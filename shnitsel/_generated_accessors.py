...
from shnitsel.analyze.spectra import apply_gauss_broadening, get_fosc, get_spectra
...
from shnitsel.clean.common import TrajectoryOrFrames, omit, transect, truncate
...
class DatasetAccessor(DSManualAccessor):
    ...
    def get_fosc(self) -> xarray.core.dataarray.DataArray | shnitsel.data.tree.tree.ShnitselDBRoot[DataArray]:
        """Wrapper for :py:func:`shnitsel.analyze.spectra.get_fosc`."""
        return get_fosc(self._obj)

    def apply_gauss_broadening(self, fosc: DataArray, agg_dim: str | None=None, width_in_eV: float=0.5, nsamples: int=1000, min_energy_range: float=0, max_energy_range: float | None=None) -> DataArray:
        """Wrapper for :py:func:`shnitsel.analyze.spectra.apply_gauss_broadening`."""
        return apply_gauss_broadening(self._obj, fosc, agg_dim=agg_dim, width_in_eV=width_in_eV, nsamples=nsamples, min_energy_range=min_energy_range, max_energy_range=max_energy_range)
