...
from shnitsel.clean.filter_energy import EnergyFiltrationThresholds, calculate_energy_filtranda, filter_by_energy
from shnitsel.clean.filter_geo import GeometryFiltrationThresholds, calculate_bond_length_filtranda, filter_by_length
...
from shnitsel.data.multi_indices import assign_levels, dtype_NA, expand_midx, flatten_levels, mdiff, mgroupby, msel, sel_trajs, stack_trajs, unstack_trajs
...
class DataArrayAccessor(DAManualAccessor):
    ...
    def keep_norming(self, exclude: (Collection | None) = None) -> DataArray:
    ...
    @needs(dims={'atom', 'direction'}, coords_or_vars={'atNames'}, not_dims={'frame'})
    def to_mol(self, charge: (int | None) = None, covFactor: float = 1.2, to2D: bool = True, molAtomMapNumber: (list | Literal[True] | None) = None, atomNote: (list | Literal[True] | None) = None, atomLabel: (list | Literal[True] | None) = None) -> Mol:
    ...
    def default_mol(self, to2D: bool = True, charge: (int | float | None) = None, molAtomMapNumber: (list | Literal[True] | None) = None, atomNote: (list | Literal[True] | None) = None, atomLabel: (list | Literal[True] | None) = None, silent_mode: bool = False) -> Mol:
    ...
    def convert_socs(self, to: str, convert_from: (str | None) = None):
        ...
        return convert_socs(self._obj, to, convert_from = convert_from)
    ...
    @needs(dims={'atom', 'direction'})
    def kabsch(self, reference_or_indexers: (DataArray | dict | None) = None, **indexers_kwargs) -> DataArray:
    ...
    def filter_data_at_hops(self, hop_type_selection: (StateSelection | Sequence | Sequence | str | None) = None) -> (DataSeries | DataArray | TreeNode[Any, DataSeries] | TreeNode[Any, DataArray]):
...
class DatasetAccessor(DSManualAccessor):
    ...
    def calc_classical_populations(self) -> (PopulationStatistics | TreeNode[Any, PopulationStatistics]):
    ...
    def sanity_check(self, filter_method: (Literal["truncate", "omit", "annotate"] | float) = 'truncate', energy_thresholds: (dict | EnergyFiltrationThresholds | None) = None, geometry_thresholds: (dict | GeometryFiltrationThresholds | None) = None, plot_thresholds: (bool | Sequence) = False, plot_populations: Literal["independent", "intersections", False] = False, mol: (Mol | None) = None, drop_empty_trajectories: bool = False) -> (TreeNode | TrajectoryOrFrames | None):
    ...
    def calculate_bond_length_filtranda(self, geometry_thresholds: (dict | GeometryFiltrationThresholds | None) = None, mol: (Mol | None) = None) -> DataArray:
    ...
    def write_ase_db(self, db_path: str, db_format: (Literal["spainn", "schnet"] | None) = None, keys_to_write: (Collection | None) = None, preprocess: bool = True, force: bool = False):
